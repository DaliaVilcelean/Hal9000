...
STATUS
(__cdecl RecomputePriorityWaitingList) (
...
STATUS
(__cdecl RecomputePriorityMutexes) (
    IN      PLIST_ENTRY     ListEntry,
    IN_OPT  PVOID           FunctionContext
    )
{
    PMUTEX  waitedMutex = CONTAINING_RECORD(ListEntry, MUTEX, AcquiredMutexListElem);
    ForEachElementExecute(&waitedMutex->WaitingList, RecomputePriorityWaitingList, FunctionContext, FALSE);
    return STATUS_SUCCESS;
}
...
)
{
    ...
    ForEachElementExecute(&Thread->AcquiredMutexesList, RecomputePriorityMutexes, &priorityDonationContext, FALSE);
